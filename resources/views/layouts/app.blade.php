...
<aside class="sidebar">
    ...
    <nav>
        ...
        <a href="{{ route('orders.index') }}" class="{{ request()->routeIs('orders.*') && !request()->routeIs('orders.archived') ? 'active' : '' }}">📋 Pedidos</a>
        @can_role(['Admin','Ventas'])
        <a href="{{ route('orders.archived') }}" class="{{ request()->routeIs('orders.archived') ? 'active' : '' }}">🗃 Archivados</a>
        @end_can_role
        @can_role(['Admin'])
